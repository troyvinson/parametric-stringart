...
// Applies a 2mm fillet to blunt the sharp bottom point of the heart.
...
module raw_string_pattern(w, h) {
    ...
    z_start = (string_layers == 1) ? z_bottom + z_usable/2 : z_bottom;
    z_step  = (string_layers > 1)  ? z_usable / (string_layers - 1) : 0;

    union() {
        for (r = [0 : string_layers - 1]) {
            z_pos = z_start + r * z_step;
            rot_offset = (string_weave && r % 2 != 0) ? (360/string_count)/2 : 0;

            translate([0, string_center_y, z_pos]) {
                for (s = [0 : string_count - 1]) {
                    rotate([0, 0, s * (360/string_count) + rot_offset])
...
module center_shape_solid() {
    scale_factor = (content_scale / 100);

    is_text_mode = is_undef(center_mode) ? false : (center_mode == "text");

    // Estimated half-width of the main text block for emoji positioning.
    // When text_string is empty, emojis center at x=0.
    _half_w = (text_string == "") ? 0
            : (len(text_string) * text_font_size * content_spacing * 0.6) / 2;
    _pre_x  = ((text_string == "" && text_suffix == "") ? 0 : -(_half_w + text_font_size * 0.6)) + emoji_prefix_x;
    _post_x = ((text_prefix == "" && text_string == "") ? 0 :  (_half_w + text_font_size * 0.6)) + emoji_suffix_x;
        ...
        if (is_text_mode) {
            scale([scale_factor, scale_factor]) {
                if (content_relief > 0) {
                        ...
                        color(content_halo_color)
                            difference() {
                                ...
                                translate([0, 0, frame_depth/2 - 1.2])
                                    linear_extrude(height=1.2 + 1) // +1 for clean boolean cut
                                        _all_text_2d_raw();
                            }

                        color(content_color)
                            translate([0, 0, frame_depth/2 - 1.2])
                                linear_extrude(height=1.2 + content_relief)
                                    _all_text_2d_raw();
                    }
                    ...
                    // Deboss — cavity is cut content_relief mm deep into the halo base,
                    ...
                    union() {
                        difference() {
                            ...
                            translate([0, 0, frame_depth/2 + content_relief - 0.1])
                                linear_extrude(height=abs(content_relief) + 1) // +1 for clean boolean cut
                                    _all_text_2d_raw();
                        }

                        color(content_color)
                            translate([0, 0, frame_depth/2 + content_relief - 1.2])
                                linear_extrude(height=1.2)
                                    _all_text_2d_raw();
                    }
                }
            ...
            if (is_undef(svg_file) || svg_file == "default.svg" || svg_file == "") {
                // Placeholder shown before an SVG is uploaded.
                color("gray")
                    linear_extrude(height=frame_depth, center=true)
                        circle(d=40);
                ...
                // Render the uploaded SVG centered and scaled.
                linear_extrude(height=frame_depth, center=true) {
                    scale([scale_factor, scale_factor]) {
