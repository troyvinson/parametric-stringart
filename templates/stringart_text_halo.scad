...
_text_w_est = (abs(_pre_x_est) + _half_w_est + abs(_post_x_est) + text_font_size) * (content_scale / 100);
_text_h_est = text_font_size * (content_scale / 100);

frame_width  = _text_w_est + (halo_offset + frame_thickness) * 2;
frame_height = _text_h_est + (halo_offset + frame_thickness) * 2;
...
module outer_profile() {
    translate([content_offset_x, content_offset_y])
        offset(r = halo_offset + frame_thickness)
            _frame_text_glyphs_2d();
}

module inner_profile() {
    translate([content_offset_x, content_offset_y])
        offset(r = halo_offset)
            _frame_text_glyphs_2d();
}

// Flat base stand so the halo frame can stand upright on a desk.
module integrated_stand() {
    // Bottom of the outer profile, estimated from font metrics.
    bottom_y = (content_offset_y - _text_h_est / 2) - (halo_offset + frame_thickness);

    stand_w  = (_text_w_est * 0.6) + halo_offset;
    stand_h  = 14;
    overlap  = 5;
    corner_r = 2;
